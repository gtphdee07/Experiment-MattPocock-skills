...
- **Social login (OAuth via Google/GitHub/etc.) was discussed (2026-09-15) and deliberately deferred, not rejected.** It's additive to this stack — `fastapi-users[oauth]` (`httpx-oauth`) plugs into the same session-cookie backend #18 already specs, no architecture change needed. Deferred for the same reason a hosted auth provider was rejected above: real added surface (per-provider app registration/secrets, redirect-URI/CSRF-state handling, and a genuine account-linking-by-email decision that doesn't exist yet) that v1's email/password + fastapi-users already avoids. Revisit as its own `to-spec` if signup friction becomes a real problem — it would not require reopening #18's email/password path, only adding to it.
