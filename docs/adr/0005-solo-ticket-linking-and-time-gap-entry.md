...
- `check_trailer_gvwr_overload` and `compute_derived_trailer_weight` (both in `towing_app/calculations.py`) are pure functions with no CLI/storage dependency, directly unit-tested in `tests/test_calculations.py`, per issue #9's explicit acceptance criterion.
